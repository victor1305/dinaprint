...
const MainSwiperImages: React.FC<SlidesProps> = ({
  slides
}): React.ReactNode => (
  <Swiper
    centeredSlides={true}
  ...
  >
    {slides.map((slide, index) => (
      <SwiperSlide key={index}>
        <div
          style={{ backgroundImage: `url(${slide.backImage})` }}
          className="h-[464px] bg-center"
        >
          <div className="!animate-swip-main p-8 h-full mx-auto lg:max-w-[360px] flex justify-center flex-col items-center">
            <h4 className="text-[35px] font-bold text-white pb-4 text-center">
              {slide.title}
            </h4>
            <Image
              src={slide.image}
