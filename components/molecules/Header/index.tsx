	...
	const menuPaths = [
		...
		},
		{
			...
			subsection: [
				...
				{ name: "CARTAS Y MENÚS", path: "cartas-y-menus" },
			...
			<nav className="flex bg-white justify-between items-center p-2.5 lg:p-5">
				...
				<div className="hidden lg:flex">
					{menuPaths.map((elm) => (
						<div
							...
							onMouseEnter={() => elm.subsection.length > 0 && setCatalogSubmenu(true)}
							onMouseLeave={() => elm.subsection.length > 0 && setCatalogSubmenu(false)}
							onFocus={() => elm.subsection.length > 0 && setCatalogSubmenu(true)}
							onBlur={() => elm.subsection.length > 0 && setCatalogSubmenu(false)}
						>
				...
				<div className="lg:hidden pr-2.5">
					{openMenu ? (
						...
						<FiMenu className="cursor-pointer w-6 h-6" onClick={() => setOpenMenu(!openMenu)} />
					)}
					<div
